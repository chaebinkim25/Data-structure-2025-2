...
def build_one_page_quiz(output_path="doc/자료구조_4주차_8회차_미니퀴즈.docx"):
    doc = Document()
    ...
    # 문항
    q = 1
    add_short(doc, q, "lqueue_enqueue()의 C 코드를 적어보시오.", lines=2); q+=1
    add_short(doc, q, "lqueue_dequeue()의 C 코드를 적어보시오.", lines=2); q+=1
    add_short(doc, q, "simple_aqueue_enqueue()의 C 코드를 적어보시오.", lines=2); q+=1
    add_short(doc, q, "simple_aqueue_dequeue()의 C 코드를 적어보시오..", lines=2); q+=1
    add_short(doc, q, "circular_aqueue_enqueue()의 C 코드를 적어보시오.", lines=2); q+=1
    add_short(doc, q, "circular_aqueue_dequeue()의 C 코드를 적어보시오.", lines=2); q+=1
    add_short(doc, q, "find()의 C 코드를 적어보시오.", lines=2); q+=1
