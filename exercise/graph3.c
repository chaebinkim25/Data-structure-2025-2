    ...
    if (n != NULL && n->status == 0) {
        ...
        while (e != NULL) {
            ...
            if (neighbor->status == 0) {
                ...
                print_graph_3col(g);
                dfs(g, neighbor, n);
                if (neighbor->min_back < n->min_back)
                    n->min_back = neighbor->min_back;

               printf("%c edge %c done. n_children = %d, min_back = %d, dfs_time = %d\n", n->data, neighbor->data, n_children, n->min_back, n->dfs_time);
                if (neighbor->min_back >= n->dfs_time && p != NULL) {
                    n->is_ap = 1;
                    print_graph_3col(g);
                }
            }
                ...
                e->edge->dfs_type = 2;
                if (neighbor->dfs_time < n->min_back)
                    n->min_back = neighbor->dfs_time;
                print_graph_3col(g);
            }
            else {
        ...
        printf("%c node done. n_children = %d, min_back = %d, dfs_time = %d\n", n->data, n_children, n->min_back, n->dfs_time);
