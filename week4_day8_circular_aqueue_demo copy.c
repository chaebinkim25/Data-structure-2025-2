...
void print_q_update_front_inverse(unsigned int front, unsigned int rear, unsigned int old_front)
{
        struct Color background = { .bg = BG_WHITE, .fg = FG_WHITE };
        set_color(&background);
...
void print_q_update_rear_inverse(unsigned int front, unsigned int rear, unsigned int old_rear)
{
        struct Color background = { .bg = BG_WHITE, .fg = FG_WHITE };
        set_color(&background);
...
void print_q_update_rear_and_front_start_inv(unsigned int front, unsigned int rear, unsigned int old_front, unsigned int old_rear)
{
        struct Color background = { .bg = BG_WHITE, .fg = FG_WHITE };
        set_color(&background);
...
void print_q_update_rear_and_front_end_inv(unsigned int front, unsigned int rear, unsigned int old_front, unsigned int old_rear)
{
        struct Color background = { .bg = BG_WHITE, .fg = FG_WHITE };
        set_color(&background);
...
static int wait(int seed)
{
        ++seed;
        ++seed;
        for (int i = 1; i < 20000; i++)                 // change 10000 to a larger number to slow down
...
{
        wait(0);
        if (!inverse && (q->front == q->rear + 1 || q->front == q->rear + 2)) {
                inverse = 1;
                print_q_update_rear_and_front_start_inv(q->front, q->rear, old_front, q->rear);
        }
        else if (inverse && (q->front == q->rear || q->front == q->rear - 1 || q->front == q->rear - 2)) {
                inverse = 0;
                print_q_update_rear_and_front_end_inv(q->front, q->rear, old_front, q->rear);                
        }
        else if (inverse) {
                print_q_update_front_inverse(q->front, q->rear, old_front);
        }
        else {
...
void aqueue_print_update_rear(struct aqueue* q, int old_rear)
{
        ...
        if (!inverse && (q->front == q->rear + 1 || q->front == q->rear + 2)) {
                inverse = 1;
                print_q_update_rear_and_front_start_inv(q->front, q->rear, q->front, old_rear);
        }
        else if (inverse && (q->front == q->rear || q->front == q->rear - 1 || q->front == q->rear - 2)) {
                inverse = 0;
                print_q_update_rear_and_front_end_inv(q->front, q->rear, q->front, old_rear);
        }
        else if (inverse){
                print_q_update_rear_inverse(q->front, q->rear, old_rear);
        }
        else {
...
void circular_aqueue_enqueue_visual(struct aqueue *q, int v)
{
        if (!q) exit(1);
...
void aqueue_enqueue_demo(struct aqueue *q)
{
        ...
        for (int i = 1; i <= 5; i++)
                circular_aqueue_enqueue_visual(q, data++);
}
